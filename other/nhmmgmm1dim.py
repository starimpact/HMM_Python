...
#            print alpha_mat
    ...
    def __calcMeanSigma(self, obs_data, gamma_list, allprob, gmi, si):
        n_obs = len(obs_data)
        #calc new mean
        meanall = 0.0
        wgtall = 0
...
#                tmp1 = postgsm
        ...
        for i in xrange(n_obs):
            ...
            gamma_row = gamma_one[si, :]
            probobs = allprob[i][si]
            for oi in xrange(len(obs_one)):
                diff = mean_new - obs_one[oi]
                probgmm = probobs[oi]
                postgsm = probgmm[gmi] / np.sum(probgmm)
                tmp1 = gamma_row[oi] * postgsm
#                tmp1 = postgsm
                sigmaall += tmp1 * diff * diff
        sigma_new = sigmaall / wgtall
        
        return mean_new, sigma_new, wgtall
...
#                print 'gmi:', gmi
                mean_new, sigma_new, wgtall = self.__calcMeanSigma(obs_data, gamma_list, allprob, gmi, si)
            ...
            gmm_new = siGMM1D(gmlist, wgtlist)
